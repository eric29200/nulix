#include <fs/fs.h>
#include <fs/minix_fs.h>
#include <proc/sched.h>
#include <drivers/char/tty.h>
#include <stderr.h>
#include <fcntl.h>

/*
 * Test if a name matches a directory entry.
 */
static inline int minix_name_match(const char *name1, size_t len1, const char *name2, size_t max_len)
{
	/* check overflow */
	if (len1 > max_len)
		return 0;

	return strncmp(name1, name2, len1) == 0 && (len1 == max_len || name2[len1] == 0);
}

/*
 * Find an entry inside a directory.
 */
static struct buffer_head *minix_find_entry(struct inode *dir, const char *name, int name_len, struct minix_dir_entry **res_de)
{
	struct minix_sb_info *sbi = minix_sb(dir->i_sb);
	int nr_entries, nr_entries_per_block, i;
	struct buffer_head *bh = NULL;
	struct minix3_dir_entry *de3;
	struct minix_dir_entry *de;
	char *namx, *p;

	/* check file name length */
	if (name_len <= 0 || name_len > sbi->s_name_len)
		return NULL;

	/* compute number of entries in directory */
	nr_entries = dir->i_size / sbi->s_dirsize;
	nr_entries_per_block = dir->i_sb->s_blocksize / sbi->s_dirsize;

	/* walk through all entries */
	for (i = 0; i < nr_entries; i++) {
		/* read next block if needed */
		if (i % nr_entries_per_block == 0) {
			/* release previous block */
			brelse(bh);

			/* get next block */
			bh = minix_bread(dir, i / nr_entries_per_block, 0);
			if (!bh)
				return NULL;
		}

		/* get directory entry */
		p = (char *) (bh->b_data + (i % nr_entries_per_block) * sbi->s_dirsize);
		if (sbi->s_version == MINIX_V3) {
			de3 = (struct minix3_dir_entry *) p;
			namx = de3->d_name;
		} else {
			de = (struct minix_dir_entry *) p;
			namx = de->d_name;
		}

		/* name match */
		if (minix_name_match(name, name_len, namx, sbi->s_dirsize)) {
			*res_de = (struct minix_dir_entry *) p;
			return bh;
		}
	}

	/* free block buffer */
	brelse(bh);
	return NULL;
}

/*
 * Add an entry to a directory.
 */
static int minix_add_entry(struct inode *dir, const char *name, int name_len, struct inode *inode)
{
	struct minix_sb_info *sbi = minix_sb(dir->i_sb);
	int nr_entries, nr_entries_per_block, i;
	struct minix3_dir_entry *mde3 = NULL;
	struct minix_dir_entry *mde = NULL;
	struct buffer_head *bh = NULL;
	char *de, *de_name;
	ino_t de_ino;

	/* check file name */
	if (name_len <= 0 || name_len > sbi->s_name_len)
		return -EINVAL;

	/* compute number of entries in directory */
	nr_entries = dir->i_size / sbi->s_dirsize;
	nr_entries_per_block = dir->i_sb->s_blocksize / sbi->s_dirsize;

	/* walk through all entries */
	for (i = 0; i <= nr_entries; i++) {
		/* read next block if needed */
		if (i % nr_entries_per_block == 0) {
			/* release previous block */
			brelse(bh);

			/* get next block */
			bh = minix_bread(dir, i / nr_entries_per_block, 1);
			if (!bh)
				return -EIO;
		}

		/* last entry : update directory size */
		if (i == nr_entries) {
			dir->i_size = (i + 1) * sbi->s_dirsize;
			mark_inode_dirty(dir);
		}

		/* get directory entry */
		de = bh->b_data + (i % nr_entries_per_block) * sbi->s_dirsize;
		if (sbi->s_version == MINIX_V3) {
			mde3 = (struct minix3_dir_entry *) de;
			de_ino = mde3->d_inode;
			de_name = mde3->d_name;
		} else {
			mde = (struct minix_dir_entry *) de;
			de_ino = mde->d_inode;
			de_name = mde->d_name;
		}

		/* found a free entry */
		if (!de_ino)
			goto found_free_entry;
	}

	/* free block buffer */
	brelse(bh);
	return -EINVAL;
found_free_entry:
	/* set new entry */
	memset(de_name, 0, sbi->s_name_len);
	memcpy(de_name, name, name_len);

	/* set new entry inode */
	if (sbi->s_version == MINIX_V3)
		mde3->d_inode = inode->i_ino;
	else
		mde->d_inode = inode->i_ino;

	/* mark buffer dirty and release it */
	mark_buffer_dirty(bh);
	brelse(bh);

	/* update parent directory */
	dir->i_mtime = dir->i_ctime = CURRENT_TIME;
	mark_inode_dirty(dir);

	return 0;
}

/*
 * Check if a directory is empty.
 */
static int minix_empty_dir(struct inode *dir)
{
	struct minix_sb_info *sbi = minix_sb(dir->i_sb);
	int nr_entries, nr_entries_per_block, i;
	struct buffer_head *bh = NULL;
	ino_t ino;
	char *de;

	/* check if dir is a directory */
	if (S_ISREG(dir->i_mode))
		return 0;

	/* compute number of entries in directory */
	nr_entries = dir->i_size / sbi->s_dirsize;
	nr_entries_per_block = dir->i_sb->s_blocksize / sbi->s_dirsize;

	/* walk through all entries */
	for (i = 0; i < nr_entries; i++) {
		/* read next block if needed */
		if (i % nr_entries_per_block == 0) {
			/* release previous block */
			brelse(bh);

			/* get next block */
			bh = minix_bread(dir, i / nr_entries_per_block, 0);
			if (!bh)
				return 0;
		}

		/* skip first 2 entries "." and ".." */
		if (i < 2)
			continue;

		/* get inode number */
		de = bh->b_data + (i % nr_entries_per_block) * sbi->s_dirsize;
		if (sbi->s_version == MINIX_V3)
			ino = ((struct minix3_dir_entry *) de)->d_inode;
		else
			ino = ((struct minix_dir_entry *) de)->d_inode;

		/* found an entry : directory is not empty */
		if (ino) {
			brelse(bh);
			return 0;
		}
	}

	/* free block buffer */
	brelse(bh);
	return 1;
}

/*
 * Lookup for a file in a directory.
 */
struct dentry *minix_lookup(struct inode *dir, struct dentry *dentry)
{
	struct minix_dir_entry *de;
	struct inode *inode = NULL;
	struct buffer_head *bh;
	ino_t ino;

	/* find entry */
	bh = minix_find_entry(dir, dentry->d_name.name, dentry->d_name.len, &de);
	if (!bh)
		goto out;

	/* get inode number */
	if (minix_sb(dir->i_sb)->s_version == MINIX_V3)
		ino = ((struct minix3_dir_entry *) de)->d_inode;
	else
		ino = de->d_inode;
	brelse(bh);

	/* get inode */
	inode = iget(dir->i_sb, ino);
	if (!inode)
		return ERR_PTR(-EACCES);

out:
	d_add(dentry, inode);
	return NULL;
}

/*
 * Create a file in a directory.
 */
int minix_create(struct inode *dir, struct dentry *dentry, mode_t mode)
{
	struct inode *inode;
	int ret;

	/* create a new inode */
	inode = minix_new_inode(dir->i_sb);
	if (!inode)
		return -ENOSPC;

	/* set inode */
	inode->i_uid = current->fsuid;
	inode->i_gid = current->fsgid;
	inode->i_op = &minix_file_iops;
	inode->i_mode = S_IFREG | mode;
	mark_inode_dirty(inode);

	/* add new entry to dir */
	ret = minix_add_entry(dir, dentry->d_name.name, dentry->d_name.len, inode);
	if (ret) {
		inode->i_nlinks = 0;
		iput(inode);
		return ret;
	}

	/* instantiate dentry */
	d_instantiate(dentry, inode);

	return 0;
}

/*
 * Make a new name for a file = hard link.
 */
int minix_link(struct inode *inode, struct inode *dir, struct dentry *dentry)
{
	struct minix_dir_entry *de;
	struct buffer_head *bh;
	int ret;

	/* inode must not be a directory */
	if (S_ISDIR(inode->i_mode))
		return -EPERM;

	/* check if new file exists */
	bh = minix_find_entry(dir, dentry->d_name.name, dentry->d_name.len, &de);
	if (bh) {
		brelse(bh);
		return -EEXIST;
	}

	/* add entry */
	ret = minix_add_entry(dir, dentry->d_name.name, dentry->d_name.len, inode);
	if (ret)
		return ret;

	/* update old inode */
	inode->i_ctime = CURRENT_TIME;
	inode->i_nlinks++;
	mark_inode_dirty(inode);

	/* instantiate dentry */
	inode->i_count++;
	d_instantiate(dentry, inode);

	return 0;
}

/*
 * Remove a file.
 */
int minix_unlink(struct inode *dir, struct dentry *dentry)
{
	struct minix_sb_info *sbi = minix_sb(dir->i_sb);
	struct inode *inode = dentry->d_inode;
	struct minix_dir_entry *de;
	struct buffer_head *bh;
	ino_t ino;
	int ret;

	/* get directory entry */
	bh = minix_find_entry(dir, dentry->d_name.name, dentry->d_name.len, &de);
	if (!bh)
		return -ENOENT;

	/* remove regular files only */
	ret = -EPERM;
	if (S_ISDIR(inode->i_mode))
		goto out;

	/* get inode number */
	if (sbi->s_version == MINIX_V3)
		ino = ((struct minix3_dir_entry *) de)->d_inode;
	else
		ino = de->d_inode;

	/* check inode */
	if (ino != inode->i_ino)
		goto out;

	/* reset directory entry */
	memset(de, 0, sbi->s_dirsize);
	mark_buffer_dirty(bh);

	/* update directory */
	dir->i_ctime = dir->i_mtime = CURRENT_TIME;
	mark_inode_dirty(dir);

	/* update inode */
	inode->i_ctime = dir->i_ctime;
	inode->i_nlinks--;
	mark_inode_dirty(inode);

	/* delete dentry */
	d_delete(dentry);
	ret = 0;
out:
	brelse(bh);
	return ret;
}

/*
 * Create a symbolic link.
 */
int minix_symlink(struct inode *dir, struct dentry *dentry, const char *target)
{
	struct minix_dir_entry *de;
	struct buffer_head *bh;
	struct inode *inode;
	size_t i;
	int ret;

	/* create a new inode */
	inode = minix_new_inode(dir->i_sb);
	if (!inode)
		return -ENOSPC;

	/* set new inode */
	inode->i_uid = current->fsuid;
	inode->i_gid = current->fsgid;
	inode->i_op = &minix_symlink_iops;
	inode->i_mode = S_IFLNK | (0777 & ~current->fs->umask);
	mark_inode_dirty(inode);

	/* get/create first block */
	bh = minix_bread(inode, 0, 1);
	if (!bh) {
		inode->i_nlinks = 0;
		iput(inode);
		return -ENOSPC;
	}

	/* write file name on first block */
	for (i = 0; target[i] && i < inode->i_sb->s_blocksize - 1; i++)
		bh->b_data[i] = target[i];
	bh->b_data[i] = 0;
	mark_buffer_dirty(bh);
	brelse(bh);

	/* update inode size */
	inode->i_size = i;
	mark_inode_dirty(inode);

	/* check if file exists */
	bh = minix_find_entry(dir, dentry->d_name.name, dentry->d_name.len, &de);
	if (bh) {
		brelse(bh);
		inode->i_nlinks = 0;
		iput(inode);
		return -EEXIST;
	}

	/* add entry */
	ret = minix_add_entry(dir, dentry->d_name.name, dentry->d_name.len, inode);
	if (ret) {
		inode->i_nlinks = 0;
		iput(inode);
		return ret;
	}

	/* instantiate dentry */
	d_instantiate(dentry, inode);

	return 0;
}

/*
 * Create a directory.
 */
int minix_mkdir(struct inode *dir, struct dentry *dentry, mode_t mode)
{
	struct minix3_dir_entry *de3;
	struct minix_dir_entry *de;
	struct minix_sb_info *sbi;
	struct buffer_head *bh;
	struct inode *inode;
	int ret;

	/* check if file exists */
	bh = minix_find_entry(dir, dentry->d_name.name, dentry->d_name.len, &de);
	if (bh) {
		brelse(bh);
		return -EEXIST;
	}

	/* allocate a new inode */
	inode = minix_new_inode(dir->i_sb);
	if (!inode)
		return -ENOSPC;

	/* set inode */
	sbi = minix_sb(dir->i_sb);
	inode->i_uid = current->fsuid;
	inode->i_gid = current->fsgid;
	inode->i_op = &minix_dir_iops;
	inode->i_mode = S_IFDIR | (mode & ~current->fs->umask & 0777);
	inode->i_nlinks = 2;
	inode->i_size = sbi->s_dirsize * 2;
	mark_inode_dirty(inode);

	/* get/create first block */
	bh = minix_bread(inode, 0, 1);
	if (!bh) {
		inode->i_nlinks = 0;
		iput(inode);
		return -ENOSPC;
	}

	/* add '.' entry */
	if (sbi->s_version == MINIX_V3) {
		de3 = (struct minix3_dir_entry *) bh->b_data;
		de3->d_inode = inode->i_ino;
		strcpy(de3->d_name, ".");
	} else {
		de = (struct minix_dir_entry *) bh->b_data;
		de->d_inode = inode->i_ino;
		strcpy(de->d_name, ".");
	}

	/* add '..' entry */
	if (sbi->s_version == MINIX_V3) {
		de3 = (struct minix3_dir_entry *) (bh->b_data + sbi->s_dirsize);
		de3->d_inode = dir->i_ino;
		strcpy(de3->d_name, "..");
	} else {
		de = (struct minix_dir_entry *) (bh->b_data + sbi->s_dirsize);
		de->d_inode = dir->i_ino;
		strcpy(de->d_name, "..");
	}

	/* release first block */
	mark_buffer_dirty(bh);
	brelse(bh);

	/* add entry to parent dir */
	ret = minix_add_entry(dir, dentry->d_name.name, dentry->d_name.len, inode);
	if (ret) {
		inode->i_nlinks = 0;
		iput(inode);
		return ret;
	}

	/* update directory links and mark it dirty */
	dir->i_nlinks++;
	mark_inode_dirty(dir);

	/* instantiate dentry */
	d_instantiate(dentry, inode);

	return 0;
}

/*
 * Remove a directory.
 */
int minix_rmdir(struct inode *dir, struct dentry *dentry)
{
	struct minix_sb_info *sbi = minix_sb(dir->i_sb);
	struct inode *inode = dentry->d_inode;
	struct minix_dir_entry *de;
	struct buffer_head *bh;
	ino_t ino;
	int ret;

	/* check if file exists */
	bh = minix_find_entry(dir, dentry->d_name.name, dentry->d_name.len, &de);
	if (!bh)
		return -ENOENT;

	/* can't remove "." */
	ret = -EPERM;
	if (inode == dir)
		goto out;

	/* must be a directory */
	ret = -ENOTDIR;
	if (!S_ISDIR(inode->i_mode))
		goto out;

	/* get inode number */
	if (sbi->s_version == MINIX_V3)
		ino = ((struct minix3_dir_entry *) de)->d_inode;
	else
		ino = de->d_inode;

	/* check inode */
	ret = -ENOENT;
	if (inode->i_ino != ino)
		goto out;

	/* directory must be empty */
	ret = -EPERM;
	if (!minix_empty_dir(inode))
		goto out;

	/* reset entry */
	memset(de, 0, sbi->s_dirsize);
	mark_buffer_dirty(bh);

	/* update dir */
	dir->i_ctime = dir->i_mtime = CURRENT_TIME;
	dir->i_nlinks--;
	mark_inode_dirty(dir);

	/* update inode */
	inode->i_ctime = dir->i_ctime;
	inode->i_nlinks = 0;
	mark_inode_dirty(inode);

	/* delete dentry */
	d_delete(dentry);
	ret = 0;
out:
	brelse(bh);
	return ret;
}

/*
 * Rename a file.
 */
int minix_rename(struct inode *old_dir, struct dentry *old_dentry, struct inode *new_dir, struct dentry *new_dentry)
{
	struct inode *old_inode = old_dentry->d_inode, *new_inode = new_dentry->d_inode;
	struct minix_sb_info *sbi = minix_sb(old_dir->i_sb);
	struct buffer_head *old_bh = NULL, *new_bh = NULL;
	struct minix_dir_entry *old_de, *new_de;
	ino_t old_ino, new_ino;
	int ret;

	/* find old entry */
	ret = -ENOENT;
	old_bh = minix_find_entry(old_dir, old_dentry->d_name.name, old_dentry->d_name.len, &old_de);
	if (!old_bh)
		goto out;

	/* get old inode number */
	if (sbi->s_version == MINIX_V3)
		old_ino = ((struct minix3_dir_entry *) old_de)->d_inode;
	else
		old_ino = old_de->d_inode;

	/* check old inode */
	if (old_inode->i_ino != old_ino)
		goto out;

	/* find new entry (if exists) or add new one */
	new_bh = minix_find_entry(new_dir, new_dentry->d_name.name, new_dentry->d_name.len, &new_de);
	if (new_bh) {
		/* get new inode number */
		if (sbi->s_version == MINIX_V3)
			new_ino = ((struct minix3_dir_entry *) new_de)->d_inode;
		else
			new_ino = new_de->d_inode;

		/* check new inode */
		ret = -ENOENT;
		if (!new_inode || new_inode->i_ino != new_ino)
			goto out;

		/* same inode : exit */
		ret = 0;
		if (old_inode->i_ino == new_inode->i_ino)
			goto out;

		/* modify new directory entry inode */
		if (sbi->s_version == MINIX_V3)
			((struct minix3_dir_entry *) new_de)->d_inode = old_inode->i_ino;
		else
			new_de->d_inode = old_inode->i_ino;

		/* update new inode */
		new_inode->i_nlinks--;
		new_inode->i_atime = new_inode->i_mtime = CURRENT_TIME;
		mark_inode_dirty(new_inode);
	} else {
		/* add new entry */
		ret = minix_add_entry(new_dir, new_dentry->d_name.name, new_dentry->d_name.len, old_inode);
		if (ret)
			goto out;
	}

	/* cancel old directory entry */
	if (sbi->s_version == MINIX_V3) {
		((struct minix3_dir_entry *) old_de)->d_inode = 0;
		memset(((struct minix3_dir_entry *) old_de)->d_name, 0, sbi->s_name_len);
	} else {
		old_de->d_inode = 0;
		memset(old_de->d_name, 0, sbi->s_name_len);
	}
	mark_buffer_dirty(old_bh);

	/* update old and new directories */
	old_dir->i_atime = old_dir->i_mtime = CURRENT_TIME;
	mark_inode_dirty(old_dir);
	new_dir->i_atime = new_dir->i_mtime = CURRENT_TIME;
	mark_inode_dirty(new_dir);

	/* update dcache */
	d_move(old_dentry, new_dentry);
	ret = 0;
out:
	brelse(old_bh);
	brelse(new_bh);
	return ret;
}

/*
 * Create a node.
 */
int minix_mknod(struct inode *dir, struct dentry *dentry, mode_t mode, dev_t dev)
{
	struct minix_dir_entry *de;
	struct buffer_head *bh;
	struct inode *inode;
	int ret;

	/* check directory */
	if (!dir)
		return -ENOENT;

	/* check if file already exists */
	bh = minix_find_entry(dir, dentry->d_name.name, dentry->d_name.len, &de);
	if (bh) {
		brelse(bh);
		return -EEXIST;
	}

	/* create a new inode */
	inode = minix_new_inode(dir->i_sb);
	if (!inode)
		return -ENOSPC;

	/* set inode */
	inode->i_uid = current->fsuid;
	inode->i_gid = current->fsgid;
	inode->i_mode = mode;
	inode->i_rdev = dev;

	/* set inode operations */
	if (S_ISREG(inode->i_mode))
		inode->i_op = &minix_file_iops;
	else if (S_ISCHR(inode->i_mode))
		inode->i_op = &chrdev_iops;
	else if (S_ISBLK(inode->i_mode))
		inode->i_op = &blkdev_iops;
	else if (S_ISFIFO(inode->i_mode))
		init_fifo(inode);

	/* mark indde dirty */
	mark_inode_dirty(inode);

	/* add new entry to dir */
	ret = minix_add_entry(dir, dentry->d_name.name, dentry->d_name.len, inode);
	if (ret) {
		inode->i_nlinks--;
		iput(inode);
		return ret;
	}

	/* instantiate dentry */
	d_instantiate(dentry, inode);

	return 0;
}
