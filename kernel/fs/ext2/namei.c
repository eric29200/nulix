...
static int ext2_empty_dir(struct inode *inode)
{
	...
	if (inode->i_size < EXT2_DIR_REC_LEN(1) + EXT2_DIR_REC_LEN(2)) {
		printk("[Ext2-fs] Bad directory size %d (inode = %d)\n", inode->i_size, inode->i_ino);
		return 1;
	}
	...
	if (!bh) {
		printk("[Ext2-fs] Bad directory (inode = %d) : no data block\n", inode->i_ino);
		return 1;
	}
	...
	if (de->d_inode != inode->i_ino || !de1->d_inode || strcmp(".", de->d_name) || strcmp("..", de1->d_name)) {
		printk("[Ext2-fs] Bad directory (inode = %d) : no '.' or '..'\n", inode->i_ino);
		return 1;
	}
	...
	while (offset < inode->i_size) {
		...
		if ((char *) de >= bh->b_data + inode->i_sb->s_blocksize) {
			...
			if (!bh) {
				printk("[Ext2-fs] Directory (inode = %d) contains a hole at offset %d\n", inode->i_ino, offset);
