...
static struct buffer_head_t *ext2_read_inode_bitmap(struct super_block_t *sb, uint32_t block_group)
{
	...
	/* load inodes bitmap */
	return bread(sb, le32toh(gdp->bg_inode_bitmap));
}
...
int ext2_free_inode(struct inode_t *inode)
{
	...
	if (inode->i_ref > 1) {
		printf("[Ext2-fs] Trying to free inode %ld with ref=%d\n", inode->i_ino, inode->i_ref);
		return -EINVAL;
	}
	...
	if (inode->i_ino < sbi->s_first_ino || inode->i_ino > le32toh(sbi->s_es->s_inodes_count)) {
		printf("[Ext2-fs] Trying to free inode reserved or non existent inode %ld\n", inode->i_ino);
		return -EINVAL;
	}
