...
static struct page_table_t *clone_page_table(struct page_table_t *src, uint32_t *phys_addr)
{
  ...
  /* create a new page table */
  ret = (struct page_table_t *) kmalloc_phys(sizeof(struct page_table_t), 1, phys_addr);
  if (!ret)
    return NULL;
...
struct page_directory_t *clone_page_directory(struct page_directory_t *src)
{
  ...
  ret = (struct page_directory_t *) kmalloc_phys(sizeof(struct page_directory_t), 1, &phys);
  if (!ret)
    return NULL;
