...
static int load_font(struct font_t *font, void *font_start, void *font_end)
{
	...
	if (header->magic[0] != PSF2_MAGIC0 || header->magic[1] != PSF2_MAGIC1 || header->magic[2] != PSF2_MAGIC2 || header->magic[3] != PSF2_MAGIC3)
			return -EINVAL;
...
struct font_t *get_default_font()
{
	...
	if (!default_font) {
		...
		if (load_font(default_font, &_binary_fonts_ter_powerline_v16n_psf_start, &_binary_fonts_ter_powerline_v16n_psf_end) != 0)
			return NULL;
	}
