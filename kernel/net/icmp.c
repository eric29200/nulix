                       ...
                       const struct sockaddr *dest_addr, size_t addrlen)
{
  ...
  /* lookup for mac address */
  arp_entry = arp_lookup(sock->dev, dest_ip);
  if (!arp_entry)
    return -EINVAL;

  /* allocate a socket buffer */
  skb = skb_alloc(sizeof(struct ethernet_header_t)
                  + sizeof(struct ip_header_t)
                  + sizeof(struct icmp_header_t)
                  + len);
  if (!skb)
  ...
  ip_build_header(skb->nh.ip_header, 0, sizeof(struct ip_header_t) + sizeof(struct icmp_header_t) + len,
                  32, 64, IP_PROTO_ICMP, sock->dev->ip_addr, dest_ip);

  /* build icmp header */
  skb->h.icmp_header = (struct icmp_header_t *) skb_put(skb, sizeof(struct icmp_header_t));
  skb->h.icmp_header->type = ICMP_TYPE_ECHO;
  skb->h.icmp_header->code = 0;
  skb->h.icmp_header->chksum = 0;
  memcpy(skb->tail, buf, len);
  skb->h.icmp_header->chksum = net_checksum(skb->h.icmp_header, sizeof(struct icmp_header_t) + len);
